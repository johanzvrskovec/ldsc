...
def __filter__(fname, noun, verb, merge_obj):
    merged_list = None
    if fname:
        def f(x, n): return x.format(noun=noun, verb=verb, fname=fname, num=n)
...
def ldscore(args, log):
    ...
    if args.annot is not None:  # read --annot
        ...
        for i, fh in enumerate(cts_fnames):
            ...
            labs = [name_breaks[i]+'_'+name_breaks[i+1]
        ...
        annot_matrix = annot_matrix[sorted(
            annot_matrix.columns, key=annot_sort_key)]
        if len(cts_colnames) > 1:
            # flatten multi-index
            annot_colnames = ['_'.join([cts_colnames[i]+'_'+b for i, b in enumerate(c)])
                              for c in annot_matrix.columns]
        else:
            annot_colnames = [cts_colnames[0] +
                              '_'+b for b in annot_matrix.columns]
    ...
    # read keep_indivs
    if args.keep:
        keep_indivs = __filter__(
            args.keep, 'individuals', 'include', array_indivs)
    else:
        keep_indivs = None
    ...
    x = np.array((args.ld_wind_snps, args.ld_wind_kb,
                  args.ld_wind_cm), dtype=bool)
    if np.sum(x) != 1:
    ...
    if args.pq_exp is not None:
        ...
        pq = np.matrix(geno_array.maf*(1-geno_array.maf)
                       ).reshape((geno_array.m, 1))
    ...
    lN = geno_array.ldScoreVarBlocks(
        block_left, args.chunk_size, annot=annot_matrix)
    col_prefix = "L2"
    file_suffix = "l2"
    ...
    if args.print_snps:
        if args.print_snps.endswith('gz'):
            print_snps = pd.read_csv(
                args.print_snps, header=None, compression='gzip')
        elif args.print_snps.endswith('bz2'):
            print_snps = pd.read_csv(
                args.print_snps, header=None, compression='bz2')
        else:
    ...
    l2_suffix = '.gz'
    log.log("Writing LD Scores for {N} SNPs to {f}.gz".format(
        f=out_fname, N=len(df)))
    ...
    if annot_matrix is not None:
        ...
        M_5_50 = np.atleast_1d(np.squeeze(
            np.asarray(np.sum(annot_matrix[ii, :], axis=0))))
    else:
        M = [geno_array.m]
    ...
    try:
        ...
        if args.bfile is not None:
            ...
            if args.annot is not None and args.extract is not None:
                raise ValueError(
                    '--annot and --extract are currently incompatible.')
