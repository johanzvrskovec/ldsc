...
numeric_cols = ['P', 'N', 'N_CAS', 'N_CON', 'Z', 'OR', 'BETA',
                'LOG_ODDS', 'INFO', 'FRQ', 'SIGNED_SUMSTAT', 'NSTUDY']
...
def parse_dat(dat_gen, convert_colname, merge_alleles, log, args):
    ...
    for block_num, dat in enumerate(dat_gen):
        ...
        wrong_types = [c for c in dat.columns if c in numeric_cols and not np.issubdtype(
...
def munge_sumstats(args, p=True):
    ...
    try:
        ...
        for field in cname_translation:
            ...
            if numk > 1:
                raise ValueError('Found {num} columns named {C}'.format(
